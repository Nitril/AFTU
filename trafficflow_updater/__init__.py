...
def main(mytimer: func.TimerRequest) -> None:
    ...
    class Road(object):
        def __init__(self, road, time):
            ...
            # CurrentFlowSSS(**road['FIS'][0]['FI'][0]['CF'][0]) if str(road['FIS'][0]['FI'][0]['CF'][0].keys()).find('SSS') > -1 else CurrentFlow(**road['FIS'][0]['FI'][0]['CF'][0])
            self.currentFlow = CurrentFlow(**road['FIS'][0]['FI'][0]['CF'][0])
            self.timeStamp = time
            self.road = {
                **vars(self.tmc), **vars(self.currentFlow), 'timeStamp': self.timeStamp}
        # def __repr__(self):
    ...
    class Roads(object):
        def __init__(self, jsonObj):
            ...
            self.roads = [Road(road, self.timeStamp)
                          for road in jsonObj['RWS'][0]['RW']]
